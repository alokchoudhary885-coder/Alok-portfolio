...
export default function AboutSection() {
  ...
  return (
    <section id="about" className="relative py-24 px-4 md:px-8 bg-[#030712] border-t border-white/5 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center max-w-5xl mx-auto">
          ...
          >
            <div className="relative w-60 sm:w-68 h-72 sm:h-[360px] rounded-3xl overflow-hidden glass-card-purple p-2 group shadow-neon-purple">
              <div className="w-full h-full rounded-2xl overflow-hidden relative border border-white/15 bg-slate-950">
                ...
                <div className="absolute bottom-3 left-3 right-3 p-2.5 rounded-xl glass-panel border border-white/10 text-center font-mono">
                  <span className="font-syne font-bold text-xs text-white block">ALOK CHOUDHARY</span>
                  <span className="text-[10px] text-cyan-400 font-semibold block mt-0.5">FULL STACK DEVELOPER</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="lg:col-span-7 flex flex-col justify-center gap-6 text-center lg:text-left"
          >
            <p className="text-slate-200 font-syne font-semibold text-lg sm:text-2xl leading-relaxed">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-2.5 pt-2">
              {techPills.map((pill, idx) => (
                <span
                  key={idx}
                  className={`px-4 py-1.5 rounded-full font-mono text-xs font-bold border ${pill.color}`}
                >
                  {pill.name}
                </span>
              ))}
